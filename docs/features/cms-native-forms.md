...
The submit handler reloads the latest published site snapshot, derives the form snapshot from the published page tree, validates fields against the target `DataTable`, and creates a `data_rows` record with `createDataRow`.
